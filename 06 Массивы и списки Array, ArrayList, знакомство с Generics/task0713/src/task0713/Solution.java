...
public class Solution {
    ...
    public static void printList(ArrayList<Integer> list) {
        ...
        ArrayList <Integer> listShare3 = new ArrayList<>();
        ArrayList <Integer> listShare2 = new ArrayList<>();
        ArrayList <Integer> listOther = new ArrayList<>();

        for (int i = 0; i < list.size(); i++) {
            if (list.get(i) % 3 == 0) {
                listShare3.add(list.get(i));
            }else if (list.get(i) % 2 == 0) {
                listShare2.add(list.get(i));
            }
            else listOther.add(list.get(i));
        }

        for (int x : listShare3) {
            System.out.println(x);
        }
        for (int x : listShare2) {
            System.out.println(x);
        }
        for (int x : listOther) {
            System.out.println(x);
